...
4. 实现一个 OpenAI-compatible non-streaming provider，默认真实模型示例使用 DeepSeek V4 Flash。
...
如果没有配置 `DEEPSEEK_API_KEY` 或 `OPENAI_API_KEY`，CLI 会使用本地 mock provider。如果配置了 `DEEPSEEK_API_KEY`，CLI 会调用 DeepSeek 的 OpenAI-compatible `/chat/completions` 接口，默认模型是 `deepseek-v4-flash`。
...
```text
没有 DEEPSEEK_API_KEY / OPENAI_API_KEY -> mock provider
有 DEEPSEEK_API_KEY                    -> DeepSeek V4 Flash provider
有 OPENAI_API_KEY                      -> OpenAI fallback provider
```
...
### 4. DeepSeek V4 Flash 用环境变量配置

真实模型调用的主示例使用 DeepSeek V4 Flash。DeepSeek API 兼容 OpenAI chat completions 形态，所以 harness 里仍然只需要维护一个 OpenAI-compatible provider。

DeepSeek 示例通过三个环境变量配置：

```bash
DEEPSEEK_API_KEY=...
DEEPSEEK_BASE_URL=https://api.deepseek.com
MINI_HARNESS_MODEL=deepseek-v4-flash
```

其中只有 `DEEPSEEK_API_KEY` 是必需的。`DEEPSEEK_BASE_URL` 和 `MINI_HARNESS_MODEL` 都有默认值。

保留 `OPENAI_API_KEY` / `OPENAI_BASE_URL` 是为了说明 provider 层不绑定单一供应商；但文章和 demo 的主路径会用 DeepSeek V4 Flash，因为它适合作为低成本教学示例。
...
`createModelProviderFromEnv()` 会按顺序检查环境变量：

- 有 `DEEPSEEK_API_KEY`：返回 DeepSeek provider，默认 `deepseek-v4-flash`。
- 没有 DeepSeek key 但有 `OPENAI_API_KEY`：返回 OpenAI fallback provider。
- 都没有：返回 mock provider。
...
```bash
DEEPSEEK_API_KEY=... npm run dev -- "explain what an agent harness is"
```
...
- 能在没有 API key 时离线运行。
- 能在有 `DEEPSEEK_API_KEY` 时调用 DeepSeek V4 Flash。
